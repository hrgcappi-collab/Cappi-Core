...
def render(d, live=False, блоки=None, точка=None):
    """Итоги дня в стиле «сводка»: одна мысль — одна строка.

    Статус только там, где есть цель (выручка, время). Никаких пояснений
    в тексте: цифру, которую нельзя понять без пояснения, надо не
    пояснять, а переименовать. Раньше экран был 1 244 символа и 42
    строки, из которых 17% — курсив «почему так»; теперь ~500 и 16.
    """
    p = d["продажи"]
    пиу = d.get("выручка") or {}
    вс = пиу.get("всего", p["всего"]["сумма"])
    m = money
    ст = lambda x: "🟢" if x >= 100 else "🟡" if x >= 85 else "🔴"
    # Личные настройки: какие блоки показывать и чья точка интересна.
    # Пусто — значит всё; так сводка для тех, кто ничего не настраивал,
    # не отличается от прежней.
    нужен = lambda блок: not блоки or блок in блоки
    # Своя точка — первая и выделенная, но чужие видны: итог в шапке
    # считается по обеим, и прятать под ним половину слагаемых нельзя.
    своя = lambda т: bool(точка) and точка != "обе" and т == точка
    вперёд = lambda пары: sorted(пары, key=lambda x: (not своя(x[0]), -_вес(x[1])))
    строки = [f"<b>{'Сейчас' if live else 'Итоги'} · {d['день']:%d.%m}</b>"]

    # ── выручка
    if not нужен("выручка"):
        pass
    elif d["план"]:
        проц = вс / d["план"] * 100
        строки.append(f"{ст(проц)} <b>{m(вс)} ₴</b> · {проц:.0f}% плана")
    else:
    ...
    for т, сумма in вперёд(по_точкам.items()):
        пл = план_кор.get(т)
        if нужен("выручка"):
            имя = f"<b>{т}</b>" if своя(т) else т
            строки.append(f"    {имя} {m(сумма)}"
                          + (f" · {сумма / пл * 100:.0f}%" if пл else ""))
    if нужен("выручка") and d.get("месяц_план"):
        факт, план_нак = d["месяц_факт"], d["месяц_план"]
        строки.append(f"Месяц {m(факт)} из {m(план_нак)} · {факт / план_нак * 100:.0f}%")
    if нужен("выручка"):
        строки.append("")
    ...
    блюд = p["блюда"]["штук"]
    if нужен("заказы"):
        строки.append(f"Заказов <b>{заказов}</b> · блюд {блюд:.0f}"
                      + (f" · {блюд / заказов:.1f} на заказ" if заказов else ""))
    отм = d["отмены"] if нужен("заказы") else {}
    if отм:
        причины = ", ".join(f"{k.lower().split()[0]} {n}"
                            for k, n in sorted(отм.items(), key=lambda x: -x[1]))
        строки.append(f"Отмен <b>{sum(отм.values())}</b> · {причины}")
    elif нужен("заказы"):
        строки.append("Отмен 0")
    уд = d["удаления"] if нужен("заказы") else {}
    if уд:
        всего = sum(v["штук"] for v in уд.values())
        зак = sum(v.get("заказов", 0) for v in уд.values())
        строки.append(f"Удалено <b>{всего:.0f}</b> {счёт(всего, 'блюдо', 'блюда', 'блюд').split(' ', 1)[1]}"
                      f" в {счёт(зак, 'заказе', 'заказах', 'заказах')}")
        for причина, v in sorted(уд.items(), key=lambda x: -x[1]["штук"]):
            ...
            строки.append(f"    {причина[:1].upper() + причина[1:].lower()} {v['штук']:.0f}"
                          + (f" · {m(v['сумма'])} ₴" if v["сумма"] else "")
                          + (f" — «{комм[:40]}»" if комм else ""))
    строки.append("")
    ак = d.get("акции") or {}
    if нужен("акции") and ак.get("позиций"):
        строки.append(f"Акции <b>{m(ак['сумма'])} ₴</b> · {ак['доля']:.0f}% выручки"
                      + (f" · без продаж {ак['без_продаж']} из {ак['позиций']}"
                         if ак["без_продаж"] else ""))
    см = d.get("смена") or {}
    if нужен("смена") and см.get("часов"):
        строки.append(f"Смена <b>{см['людей']} чел</b> · {см['часов']:.0f} ч · "
                      f"{m(вс / см['часов'])} ₴/час"
                      + (f" · правок задним числом {len(см['правки'])}" if см.get("правки") else ""))
    ...
    if нужен("время") and вр and "ошибка" not in вр:
        for т, v in вперёд(вр.items()):
            ...
            знак = ("🟢" if ц and v["доставка"] <= ц else "🔴") if ц else ""
            строки.append(f"Время {f'<b>{т}</b>' if своя(т) else т} "
                          f"{v['кухня']:.0f}·{v['админ']:.0f}·{v['курьер']:.0f} = "
                          f"<b>{v['доставка']:.0f} мин</b>"
                          + (f" {знак} цель {ц:.0f}" if ц else ""))
    ж = d.get("жалобы") or {}
    if нужен("жалобы") and ж and "ошибка" not in ж:
        крит = ж["срочность"].get("critical", 0) + ж["срочность"].get("high", 0)
        строки.append(f"Жалоб <b>{ж['жалоб']}</b>"
                      + (f" из {ж['отзывов']}" if ж.get("отзывов") else "")
                      + (f" · срочных {крит}" if крит else ""))
    з = d.get("зоны") or {}
    if нужен("жалобы") and з.get("закрытий"):
        строки.append(f"Зоны закрывали {счёт(з['закрытий'], 'раз', 'раза', 'раз')} · "
                      f"{з['минут'] / 60:.1f} ч")
    жв = d.get("живые") or {}
    if live and жв.get("в_работе"):
        строки += ["", f"В работе <b>{жв['в_работе']}</b> · "
                       f"{m(жв.get('сумма_в_работе', 0))} ₴"]
    # Пустая строка между группами — воздух, две подряд — дыра от
