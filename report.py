...
def _знак(факт, цель_, больше_лучше=True, жёлтый=0.85):
    """🟢 · 🟡 · 🔴 по отношению к цели. Одно правило на весь отчёт."""
    if not цель_:
        return ""
    доля = факт / цель_
    if not больше_лучше:
        доля = цель_ / факт if факт else 2
    return "🟢" if доля >= 1 else "🟡" if доля >= жёлтый else "🔴"
...
def _проблемы(d, вс, блоки=None):
    ...
    def добавить(факт, порог, текст, больше_хуже=True):
        """Тяжесть — во сколько раз показатель хуже своего красного порога.

        Сравнивать минуты с процентами и штуками напрямую нельзя, а
        «насколько перевалил за красное» — можно: это одна шкала.
        """
        доля = (факт / порог) if больше_хуже else (порог / max(факт, 0.01))
        if доля <= 0.85:
            return
        беды.append((доля, "🔴" if доля >= 1 else "🟡", текст))

    if нужен("выручка") and d.get("план"):
        проц = вс / d["план"] * 100
        if проц < 100:
            # Красный порог выручки — 85% плана.
            добавить(проц, 85, f"Выручка {проц:.0f}% плана", больше_хуже=False)
    ...
    if нужен("время") and "ошибка" not in вр:
        for т, v in вр.items():
            ...
            if ц and v["доставка"] > ц:
                # Красный порог скорости — сама цель: минута сверху уже мимо.
                добавить(v["доставка"], ц,
    ...
    if нужен("жалобы") and ж and "ошибка" not in ж and ж.get("жалоб"):
        ...
        добавить(max(доля, 0.2 if срочных else 0), 0.2,
                 f"Жалобы — {ж['жалоб']}"
...
def render(d, live=False, блоки=None, точка=None):
    ...
    if нужен("выручка"):
        ...
        if d["план"]:
            проц = вс / d["план"] * 100
            строки.append(f"{_знак(вс, d['план'])} <b>{m(вс)} ₴</b> · {проц:.0f}% плана")
        ...
        if по_точкам:
            ...
            for т, сумма in вперёд(по_точкам.items()):
                пл = план_кор.get(т)
                строки.append(f"{имя(т)} · {_знак(сумма, пл)} {m(сумма)}"
                              + (f" · {сумма / пл * 100:.0f}%" if пл else ""))
        if d.get("месяц_план"):
            факт, план_нак = d["месяц_факт"], d["месяц_план"]
            строки += ["", "📈 <b>МЕСЯЦ</b>",
                       f"{_знак(факт, план_нак)} <b>{m(факт)}</b> / {m(план_нак)} ₴ "
    ...
    if нужен("заказы"):
        ...
        if отм:
            ...
            строки.append(f"{'🔴' if доля > 0.12 else '🟡'} <b>{всего_отмен}</b>")
            строки += _ветки([(k[:1].upper() + k[1:].lower(), str(n))
    ...
    if нужен("акции") and ак.get("позиций"):
        ...
        if ак["без_продаж"]:
            доля = ак["без_продаж"] / max(ак["позиций"], 1)
            строки.append(f"{'🔴' if доля > 0.5 else '🟡'} без продаж "
    ...
    if нужен("время") and вр and "ошибка" not in вр:
        ...
        for т, v in вперёд(вр.items()):
            ц = sum(цель(k, т) or 0 for k in ("кухня", "админ", "курьер")) or None
            знак = _знак(v["доставка"], ц, больше_лучше=False, жёлтый=0.9)
    ...
    if нужен("жалобы") and ж and "ошибка" not in ж:
        ...
        строки += ["😠 <b>КЛИЕНТЫ</b>",
                   f"{'🔴' if срочных or доля > 0.2 else '🟡' if ж['жалоб'] else '🟢'} "
