...
def render(d, live=False):
    ...
    if "ошибка" in ж:
        ...
        # То же и здесь: «доставок» — это записи Cloud, а не заказы.
        строки += ["", f"🚚 Записей о доставке: <b>{ж['всего']}</b>"
                       + (f" · отменено {отменено}" if отменено else ""),
                   "<i>записей больше, чем заказов: у доставки есть "
                   "служебная строка без еды</i>"]
