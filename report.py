...
def render(d, live=False):
    ...
    p = d["продажи"]
    # Выручка берётся из ПиУ, а не из продаж: по ней считают деньги, и
    # расходиться с отчётом о прибылях и убытках отчёту дня нельзя.
    # Продажи остаются источником количеств — блюд, чеков, позиций.
    пиу = d.get("выручка") or {}
    вс = {"сумма": пиу.get("всего", d["продажи"]["всего"]["сумма"])}
    ...
    по_точкам = пиу.get("точки") or {
        ТОЧКИ.get(т, т): ф["сумма"] for т, ф in (d.get("точки") or {}).items()}
    if по_точкам:
        строки.append("")
        # План хранится под полным именем точки («Зал Лазарева»), выручка
        # приходит под коротким — сводим через ТОЧКИ.
        план_кор = {ТОЧКИ.get(т, т): v
                    for т, v in (d.get("план_точки") or {}).items()}
        for кор, сумма in sorted(по_точкам.items(), key=lambda x: -x[1]):
            ф = {"сумма": сумма}
            пл = план_кор.get(кор)
            хвост = ""
            if пл:
            ...
            строки.append(f"    {кор:<10} {money(сумма):>9} ₴{хвост}")
    ...
    строки += ["",
               f"📦 Заказов: <b>{заказов}</b>",
               f"🍱 Блюд: <b>{p['блюда']['штук']:,.0f}</b>".replace(",", " ")
               + (f" · <b>{p['блюда']['штук'] / заказов:.1f}</b> на заказ"
                  if заказов else "")]
    ...
    if d["удаления"]:
        ...
        заказов_с_удал = sum(v.get("заказов", 0) for v in d["удаления"].values())
        строки += ["", f"🗑 Удалено: <b>{всего:.0f}</b> "
                       f"{счёт(всего, 'блюдо', 'блюда', 'блюд').split(' ', 1)[1]}"
                       f" в {счёт(заказов_с_удал, 'заказе', 'заказах', 'заказах')}"]
        for причина, v in sorted(d["удаления"].items(), key=lambda x: -x[1]["штук"]):
            деньги = f" · {money(v['сумма'])} ₴" if v["сумма"] else ""
            строки.append(f"    {причина} — {v['штук']:.0f} шт "
                          f"в {v.get('заказов', 0)} зак{деньги}")
            # Комментарий объясняет причину лучше, чем её тип: «Со
            # списанием» — это категория, «клиент не забрал заказ» — факт.
            for к, n in sorted((v.get("комментарии") or {}).items(),
                               key=lambda x: -x[1])[:3]:
                строки.append(f"        <i>«{к[:52]}»{f' ×{n}' if n > 1 else ''}</i>")
    ...
    if см.get("часов"):
        ...
        # Разбивка по ролям убрана: в итогах дня она не решение, а справка,
        # и её место на экране «Смена», где рядом есть филиалы и ФИО.
        строки += ["", f"👥 Смена: <b>{см['людей']} чел</b> · "
                       f"{см['часов']:.0f} ч · "
                       f"<b>{money(на_час)} ₴</b> на человеко-час"]
        if см["правки"]:
            строки.append(f"    ⚠️ <b>правок задним числом: {len(см['правки'])}</b>")

    вр = d.get("времена") or {}
    if "ошибка" in вр:
        строки += ["", f"⚠️ Время работы недоступно: {вр['ошибка']}"]
    elif вр:
        # Сколько минут проходит от заказа до двери, и чьи это минуты.
        # Одно число «доставка 71 минута» не говорит, с кем разговаривать;
        # разбивка на кухню, сборку и дорогу — говорит.
        строки += ["", "⏱ <b>Время</b>  <i>кухня · админ · курьер = доставка</i>"]
        for точка, v in sorted(вр.items()):
            цели = [цель(k, точка) for k in ("кухня", "админ", "курьер")]
            цель_всего = sum(c or 0 for c in цели) or None
            куски = " · ".join(
                f"{v[k]:.0f}{'⚠️' if (c and v[k] > c) else ''}"
                for k, c in zip(("кухня", "админ", "курьер"), цели))
            хвост = ""
            if цель_всего:
                мимо = v["доставка"] - цель_всего
                хвост = (f"  <i>цель {цель_всего:.0f}, "
                         f"{'+' if мимо > 0 else ''}{мимо:.0f}</i>")
            строки.append(f"    {точка}: {куски} = <b>{v['доставка']:.0f} мин</b>{хвост}")
    ...
    if з.get("закрытий"):
        ...
        хвост = []
        if з["ещё_закрыты"]:
            # Было «время по плану, не факт» — фраза, которую невозможно
            # понять, не зная внутренностей. Говорим по-человечески: зона
            # закрыта прямо сейчас, и сколько она простоит, мы не знаем.
            хвост.append(счёт(з["ещё_закрыты"], "зона закрыта прямо сейчас",
                               "зоны закрыты прямо сейчас",
                               "зон закрыты прямо сейчас")
                         + ", их время посчитано по заявленному, а не по факту")
        if з["авто"]:
    ...
    if "ошибка" in ж:
        ...
        # Раньше писали «31 из 267» — и рядом «Заказов: 128». Разные вещи:
        # 128 это чеки с едой за учётный день, 267 — записи доставок за дату
        # доставки, вместе с отменёнными и предзаказами, а один заказ
        # бывает разбит на несколько чеков. Ставить их рядом значит
        # заставлять человека искать ошибку там, где её нет.
        строки += ["", f"🚚 <b>В работе сейчас: {ж['в_работе']}</b> на "
                       f"<b>{money(ж.get('сумма_в_работе', 0))} ₴</b>"]
        for st, n in sorted(ж["статусы"].items(), key=lambda x: -x[1]):
            if st not in В_РАБОТЕ:
                continue
            сумма = (ж.get("суммы") or {}).get(st) or 0
            строки.append(f"    {статус(st)} — {n}"
                          + (f" · {money(сумма)} ₴" if сумма else ""))
        # «Закрыто за день» отсюда убрано. Cloud считает документы, а не
        # заказы: у каждой доставки есть вторая служебная запись без еды, и
        # 144 настоящих заказа превращались в «246 закрытых». Проверено на
        # заказе №8909 — одно время открытия, пять блюд, а UniqOrderId по
        # нему возвращает два. Отмены уже показаны выше отдельным блоком.
    # Для прошедшего дня блок доставок не показываем вовсе: «в работе 0» —
    # не новость, а счётчик записей Cloud не сопоставим с числом заказов и
    # только сбивает с толку. Отмены уже показаны отдельным блоком выше.
