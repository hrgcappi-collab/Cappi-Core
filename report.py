...
def render(d, live=False, блоки=None, точка=None):
    ...
    своя = lambda т: not точка or точка == "обе" or т == точка
    строки = [f"<b>{'Сейчас' if live else 'Итоги'} · {d['день']:%d.%m}</b>"
              + (f" · {точка}" if точка and точка != "обе" else "")]
    ...
    for т, сумма in sorted(по_точкам.items(), key=lambda x: -x[1]):
        пл = план_кор.get(т)
        if нужен("выручка") and своя(т):
            строки.append(f"    {т} {m(сумма)}" + (f" · {сумма / пл * 100:.0f}%" if пл else ""))
    ...
    if live and жв.get("в_работе"):
        строки.append(f"В работе <b>{жв['в_работе']}</b> · {m(жв.get('сумма_в_работе', 0))} ₴")
    return "\n".join(строки)
