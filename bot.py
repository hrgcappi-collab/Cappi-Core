...
def cmd_cancels(chat, day=None):
    ...
    m = report.money
    строки = [f"<b>Отмены · {day:%d.%m}</b>"]
    if от:
        строки.append(f"<b>{sum(от.values())}</b> · " + ", ".join(
            f"{п.lower().split()[0]} {n}" for п, n in sorted(от.items(), key=lambda x: -x[1])))
    else:
        строки.append("Отмен нет")
    if уд:
        всего = sum(v["штук"] for v in уд.values())
        зак = sum(v.get("заказов", 0) for v in уд.values())
        строки.append(f"Удалено <b>{всего:.0f}</b> "
        ...
        for п, v in sorted(уд.items(), key=lambda x: -x[1]["штук"]):
            комм = next(iter(sorted((v.get("комментарии") or {}).items(), key=lambda x: -x[1])), (None,))[0]
            строки.append(f"    {э(п[:1].upper() + п[1:].lower())} {v['штук']:.0f}"
...
def cmd_stoplist(chat):
    ...
    if not позиции:
        return say(chat, "<b>Стоп-лист</b> · 🟢 пуст")
    ...
    строки = [f"<b>Стоп-лист</b> · <b>{len(сгруппировано)}</b> позиций · {report.money(потери)} ₴"]
    кнопки = []
    for имя, г in list(сгруппировано.items())[:20]:
        где = ", ".join(sorted({т for т, _ in г["где"]}))
        строки.append(f"    {э(имя[:34])} {report.money(г['цена'] or 0)} ₴ · {где}")
...
def cmd_deletions(chat, day=None):
    ...
    for причина, r in sorted(по_причинам.items(), key=lambda x: -x[1]["сумма"]):
        строки.append(f"    {э(причина[:34])} {r['раз']} · {m(r['сумма'])} ₴")
    строки.append("")
