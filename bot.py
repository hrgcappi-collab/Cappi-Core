        ...
        if p.get("weekly"):
            ...
            for точка, дни in p["weekly"].items():
                ...
                строки.append(f"  {report.ТОЧКИ.get(точка, точка)} — "
                              f"{report.money(sum(суммы))} ₴ за неделю с {пн:%d.%m}")
                строки.append("    " + "  ".join(
...
def cmd_stoplist(chat):
    ...
    строки = [f"🛑 <b>В стопе: {len(сгруппировано)} позиций</b>"
              + (f" <i>({len(позиции)} с учётом точек)</i>" if len(позиции) != len(сгруппировано) else ""),
              f"<i>на {report.money(потери)} ₴ по прайсу</i>", ""]
    кнопки = []
    for имя, г in list(сгруппировано.items())[:20]:
        где = ", ".join(sorted({т for т, _ in г["где"]}))
        строки.append(f"  {э(имя[:34])} — {report.money(г['цена'] or 0)} ₴  <i>{где}</i>")
    ...
    if access.можно(chat, "цены") and len(кнопки) >= 12:
        строки.append("<i>Кнопки «снять» — для первых двенадцати; остальные через поиск позиции.</i>")
...
def cmd_deletions(chat, day=None):
    ...
    строки += ["", "<b>Поштучно</b>"]
    # Строки по 0 ₴ («.Гарного дня!», палочки) — не потери, а шум.
    for x in sorted((x for x in сп if x["сумма"] > 0), key=lambda x: -x["сумма"])[:15]:
        кто = re.sub(r"\s*[(,].*$", "", x["кто"]).strip()[:22]
        зал = report.ТОЧКИ.get(x["зал"], x["зал"])
        строки.append(f"  {э(x['блюдо'][:30])} — {report.money(x['сумма'])} ₴"
                      f"\n     чек {x['чек']} · {зал} · {э(кто)}")
...
def cmd_shift(chat, day=None, поимённо=False):
    ...
    По филиалам, потому что смена — это не сеть, а конкретная кухня:
    девятнадцать человек «вообще» ничего не говорят, а девять на
    Лазаревой при её выручке — говорят сразу.
    """
    day = day or date.today()
    with cappi.Syrve() as s:
        см = report.attendance(s, day)
        продажи = report.sales(s, day)
        точки = report.sales_by_point(s, day)
    if not см["часов"]:
    ...
    строки = [f"👥 <b>Смена {day:%d.%m}</b>", "",
              f"{склонение(см['людей'], 'человек', 'человека', 'человек')} · "
              f"{см['часов']:.0f} ч · "
              f"<b>{report.money(выручка / см['часов'])} ₴</b> на человеко-час",
              ""]

    # Выручка филиала на его же часы: это и есть цифра, за которую
    # отвечает управляющий точкой.
    выручка_точки = {report.ТОЧКИ.get(report.СЛИВАТЬ.get(и, и)): v["сумма"]
                     for и, v in точки.items()}
    for филиал, ф in sorted(см["по_филиалам"].items(),
                            key=lambda x: -x[1]["часов"]):
        в = выручка_точки.get(филиал)
        заголовок = (f"<b>{филиал}</b> · {ф['людей']} чел · {ф['часов']:.0f} ч")
        if в:
            заголовок += f" · <b>{report.money(в / ф['часов'])} ₴</b>/час"
        строки.append(заголовок)
        for роль, r in sorted(ф["роли"].items(), key=lambda x: -x[1]["часов"]):
            строки.append(f"    {роль} — {r['людей']} чел, {r['часов']:.0f} ч")
        строки.append("")

    if поимённо:
        строки.append("<b>Поимённо</b>")
        текущий = None
        for ч in sorted(см["люди"],
                        key=lambda ч: (ч["филиал"] or "яя", -ч["часов"])):
            ф = ч["филиал"] or "без филиала"
            if ф != текущий:
                строки.append(f"<i>{ф}</i>")
                текущий = ф
            # Филиал из имени уже показан в заголовке — в строке он лишний.
            имя = re.sub(r"\s*[(,].*$", "", ч["имя"]).strip()
            строки.append(f"    {имя[:26]:<26} {ч['роль'][:18]:<18} "
                          f"{ч['часов']:>4.1f} ч  <i>{', '.join(ч['смены'][:2])}</i>")
        строки.append("")
