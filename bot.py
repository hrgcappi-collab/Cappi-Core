...
def _просить_тайники(последний):
    ...
    просьба = (f"Сколько тайных гостей прошло за <b>{имя_месяца}</b>, "
               f"по каждому филиалу?\n\n"
               f"Ответь так:\n<code>/secret Лазарева 3 4</code>\n"
               f"<code>/secret Левітана 3 4</code>\n"
               f"<i>филиал, пройдено, всего</i>")
...
def cmd_процент(chat, месяц=None):
    ...
    строки = [f"💵 <b>Процент кухни · {МЕСЯЦЫ[day.month - 1]} {day.year}</b>", "",
              f"выручка кухни {report.money(r['выручка'])} ₴",
              f"пул {kpi.ПРОЦЕНТ_КУХНИ:g}% = <b>{report.money(r['пул'])} ₴</b>",
              f"{r['часов']:.0f} часов · {report.money(r['за_час'])} ₴ за час", ""]
    for имя, v in r["люди"].items():
        правка = ""
        if v["правка"]:
            правка = f"  <i>{v['правка']:+.0f} ч</i>"
        строки.append(f"  {имя.split('(')[0].strip()[:26]:<26} "
                      f"{v['итого']:>5.0f} ч  <b>{report.money(v['сумма'])}</b> ₴{правка}")
    if (day.year, day.month) == (date.today().year, date.today().month):
        строки += ["", "⚠️ <i>Месяц не закончен — суммы предварительные.</i>"]
    строки += ["", "<i>Шеф-повар в пул не входит: у него свой KPI. "
