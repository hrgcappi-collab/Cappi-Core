...
def cmd_kpi(chat, day=None):
    """KPI шефа и су-шефов за месяц.

    Пять критериев, из них четыре считаются сами, тайный гость вносится
    руками — этих данных нет ни в одной системе.
    """
    day = day or date.today()
    say(chat, "Считаю KPI, это займёт минуту…")
    with cappi.Syrve() as s:
        r = kpi.посчитать(s, day)
    н = kpi.НОРМА
    строки = [f"🏆 <b>KPI · {МЕСЯЦЫ[day.month - 1]} {day.year}</b>", ""]

    for имя, v in sorted(r["точки"].items()):
        норма_вр = v.get("норма_времени") or н["время_шеф"]
        строки += [f"<b>Су-шеф {имя}</b>",
                   f"    выручка кухни {report.money(v['выручка'])} ₴",
                   f"    {_знак(v['время'], норма_вр)} время выдачи "
                   f"<b>{v['время']:.1f}</b> мин  <i>норма ≤{норма_вр:g}</i>",
                   f"    {_знак(v['списание_%'], н['списание'])} списание "
                   f"<b>{v['списание_%']:.2f}%</b>  <i>норма ≤{н['списание']}</i>"
                   f"  ({report.money(v['списание'])} ₴)",
                   f"    {_знак(abs(v['переучёт_%']), н['переучёт'])} переучёт "
                   f"<b>{v['переучёт_%']:.2f}%</b>  <i>норма ±{н['переучёт']}</i>"
                   f"  ({report.money(v['переучёт'])} ₴)", ""]

    строки += ["<b>Шеф-повар</b>",
               f"    {_знак(r['время_среднее'], н['время_шеф'])} среднее время "
               f"<b>{r['время_среднее']:.1f}</b> мин  <i>норма ≤{н['время_шеф']}</i>",
               f"    {_знак(r['негатив_%'], н['негатив'])} негатив по кухне "
               f"<b>{r['негатив_%']:.2f}%</b>  <i>норма ≤{н['негатив']}</i>"
               f"  ({r['негатив']} из {r['заказов']} заказов)",
               f"    {_знак(abs(r['переучёт_%']), н['переучёт'])} переучёт по сети "
               f"<b>{r['переучёт_%']:.2f}%</b>  <i>норма ±{н['переучёт']}</i>",
               f"    {_знак(r['списание_%'], н['списание'])} списание по сети "
               f"<b>{r['списание_%']:.2f}%</b>  <i>норма ≤{н['списание']}</i>", ""]

    т = r.get("тайники")
    if т:
        всего = sum(v.get("всего", 0) for v in т.values())
        пройдено = sum(v.get("пройдено", 0) for v in т.values())
        проц = пройдено / всего * 100 if всего else 0
        строки.append(f"🕵️ {_знак(проц, н['тайник'], меньше_лучше=False)} "
                      f"тайный гость <b>{проц:.0f}%</b>  <i>норма ≥{н['тайник']:g}</i>")
        for точка, v in sorted(т.items()):
            строки.append(f"    {точка} — {v.get('пройдено')} из {v.get('всего')}")
    else:
        строки.append("🕵️ <b>Тайный гость: данных нет</b>\n"
                      "    <i>вносится командой</i> <code>/secret Лазарева 3 4</code>")

    строки += ["", "<i>Выручка кухни — только блюда, без напитков и сборов. "
                   "Списание: счета 02.08+03.01+03.06. Переучёт: 02.04+02.09.</i>"]
    say(chat, "\n".join(строки))
...
def on_message(m):
    ...
    if cmd in ("/start", "/help"):
        ...
        cmd_kpi(chat, date.fromisoformat(args[0]) if args else None)
    elif cmd == "/special":
