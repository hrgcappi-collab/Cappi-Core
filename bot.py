...
def _спросить_айку(chat, текст):
    """Вопрос, на который нет экрана, но есть данные в Syrve.

    Модель не отвечает на вопрос — она составляет разрез, а считает и
    рисует его код. Поля проверяются по закрытому списку в report: всё,
    чего в нём нет, отвергается, а не исправляется на месте. Поэтому
    «сходи в айку и посмотри» не требует доверять модели ни одной цифры.
    """
    план = мозг.запрос_в_олап(текст, report.РАЗРЕЗЫ, report.СЧИТАЛКИ,
                              date.today().isoformat(), бот="core")
    if not план:
        return False
    с, по = дата_из_текста(план["с"]), дата_из_текста(план["по"])
    if not с or not по or по < с:
        return False
    _печатает(chat)
    try:
        with cappi.Syrve() as s:
            д = report.свободный_запрос(s, план["разрез"], план["считать"],
                                        с, по, план["только_еда"],
                                        план.get("отбор", ""))
    except ValueError as e:
        return say(chat, f"Так спросить не могу: {e}.") or True
    строки_ = д["строки"]
    if not строки_:
        # Пусто бывает от двух разных причин, и путать их нельзя: данных
        # правда нет — или отбор оказался не про то. Второе чаще.
        хвост = (f"\n<i>Отбор «{э(план['отбор'])}» ничего не совпал. "
                 f"Может быть, это не блюдо и не категория — тогда спроси "
                 f"без него.</i>" if план.get("отбор") else "")
        return say(chat, f"{э(план['заголовок'] or 'Запрос')} · "
                         f"{с:%d.%m}–{по:%d.%m}\nНичего не нашлось.{хвост}") or True
    главная = д["считать"][0]
    строки_.sort(key=lambda r: -(r.get(главная) or 0))
    подпись = lambda к: report.СЧИТАЛКИ.get(к, к)
    out = [f"🔎 <b>{э(план['заголовок'] or 'Из Syrve')}</b> · "
    ...
    if д["разрез"]:
        for r in строки_[:12]:
            имя = " · ".join(str(r.get(p) or "—")[:26] for p in д["разрез"])
    ...
    out.append(f"<i>Разрез: {', '.join(report.РАЗРЕЗЫ.get(p, p) for p in д['разрез']) or 'без разбивки'}. "
               f"Считал код, не модель.</i>")
    if "DishDiscountSumInt" in д["считать"]:
        # Продажи из OLAP и «Выручка» в ПиУ — разные числа, и путать их
        # нельзя: по ПиУ считают деньги. Здесь именно продажи.
...
def _разобрать_умом(chat, текст, кто):
    ...
    if not разбор or разбор["уверенность"] < мозг.ПОРОГ_УВЕРЕННОСТИ:
        ...
        # Экрана нет — но данные могут быть. Пробуем сходить в Syrve
        # разрезом, который составит модель.
        try:
            if _спросить_айку(chat, текст):
                return True
        except Exception as e:
            bugs.записать("апи", f"свободный запрос не прошёл: {str(e)[:150]}",
                          где="запрос в Syrve")
